  ...
  if (_username === targetUsername) {
    ...
    if (imgCandidates.length > 0) {
      for (const _img of imgCandidates) {
        imgs.push(
          `<NextImage ${(_img[0] as any).isCarousel ? 'carousel' : ''} src="${
            _img[0].url
          }" height={${_img[0].height ?? 250}} width={${
            _img[0].width ?? 250
          }} alt="Content Image" />`
        );
      }
...
export async function getThreads(_postPk: string) {
  ...
  if (_tr) {
    ...
    const q_t =
      _tc.thread_items[0].post.text_post_app_info.share_info.quoted_post;
    if (q_t) {
    ...
    threads.push(
      '*this is auto generated content from [ThreadsApp](https://threads.net). see original at [@gadingnst](https://threads.net/@gadingnst)*'
    );
  }
